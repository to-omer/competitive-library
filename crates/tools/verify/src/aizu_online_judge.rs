use crate::{TestCase, app_cache_directory, gen_case};
use fd_lock::RwLock;
...
pub fn get_testcases(
    ...
    let runtime = runtime::Builder::new_current_thread()
        .enable_all()
        .build()?;
    let header = problemdir.join("header.json");
    runtime.block_on(gen_case(
        format!(
    ...
    let headers: AOJTestCaseHeaders = serde_json::from_reader(File::open(header)?)?;

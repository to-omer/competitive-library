    ...
    M: Montgomery32NttModulus,
{
    ...
    while i + 16 <= a.len() {
        ...
        let y = _mm512_min_epu32(x, _mm512_sub_epi32(x, mod_vec));
...
unsafe fn add_vec_avx512<M>(a: __m512i, b: __m512i, mod_vec: __m512i, mod2_vec: __m512i) -> __m512i
where
    M: Montgomery32NttModulus,
{
...
unsafe fn sub_vec_avx512<M>(a: __m512i, b: __m512i, mod_vec: __m512i, mod2_vec: __m512i) -> __m512i
where
    M: Montgomery32NttModulus,
{
    if M::MOD < LAZY_THRESHOLD {
    ...
    M: Montgomery32NttModulus,
{
    ...
    if n.trailing_zeros() & 1 == 1 {
        ...
        while i + 16 <= v {
            ...
            let x1 = _mm512_loadu_si512(a.as_ptr().add(v + i) as *const __m512i);
            let y0 = add_vec_avx512::<M>(x0, x1, mod_vec, mod2_vec);
            let y1 = sub_vec_avx512::<M>(x0, x1, mod_vec, mod2_vec);
            _mm512_storeu_si512(a.as_mut_ptr().add(i) as *mut __m512i, y0);
            _mm512_storeu_si512(a.as_mut_ptr().add(v + i) as *mut __m512i, y1);
            i += 16;
        }
        while i < v {
            let x0 = a[i];
        ...
        for (s, block) in a.chunks_exact_mut(v << 1).enumerate() {
            ...
            while i + 16 <= half {
                ...
                let (a1, a2, a3) = if s == 0 {
                    ...
                    (
                        mul_vec_avx512::<M>(x1, w1v, r_vec, mod_vec),
                        mul_vec_avx512::<M>(x2, w2v, r_vec, mod_vec),
                        mul_vec_avx512::<M>(x3, w3v, r_vec, mod_vec),
                    )
                };
...
#[target_feature(enable = "avx512f,avx512dq,avx512cd,avx512bw,avx512vl")]
pub unsafe fn ntt_batch_avx512<M>(a: &mut [MInt<M>], width: usize)
where
    M: Montgomery32NttModulus,
{
    let n = a.len() / width;
    ...
    if n.trailing_zeros() & 1 == 1 {
        ...
        while i + 16 <= half {
            let x0 = _mm512_loadu_si512(a.as_ptr().add(i) as *const __m512i);
            let x1 = _mm512_loadu_si512(a.as_ptr().add(half + i) as *const __m512i);
            let y0 = add_vec_avx512::<M>(x0, x1, mod_vec, mod2_vec);
            let y1 = sub_vec_avx512::<M>(x0, x1, mod_vec, mod2_vec);
            _mm512_storeu_si512(a.as_mut_ptr().add(i) as *mut __m512i, y0);
            _mm512_storeu_si512(a.as_mut_ptr().add(half + i) as *mut __m512i, y1);
            i += 16;
        }
        while i < half {
            let x0 = a[i];
    ...
    M: Montgomery32NttModulus,
{
        ...
        for (s, block) in a.chunks_exact_mut(v << 2).enumerate() {
            ...
            while i + 16 <= v {
                ...
                let (y1, y2, y3) = if s == 0 {
                    ...
                    (
                        mul_vec_avx512::<M>(y1, w1v, r_vec, mod_vec),
                        mul_vec_avx512::<M>(y2, w2v, r_vec, mod_vec),
                        mul_vec_avx512::<M>(y3, w3v, r_vec, mod_vec),
                    )
                };
    ...
    let inv_vec = _mm512_set1_epi32(inv as i32);
    let mut i = 0;
    while i + 16 <= n {
        let x = _mm512_loadu_si512(a.as_ptr().add(i) as *const __m512i);
